...
# Modified date : 2019-03-23 16:08
# Author : DARREN
# Describe : not set
...
def _get_pretrained_model(config):
    ...
    if config["model_name"] == "gpt":
        ...
        model = GPT2DoubleHeadsModel.from_pretrained(model_file, config_file, config)
    model.to(device)
    return model
...
def do_train(config, model=None):
    ...
    return train_loss, model_config, model
